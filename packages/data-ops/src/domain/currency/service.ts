...
const convert = (
  ...
  fromCurrency: string,
  toCurrency: string,
): Effect.Effect<number, CurrencyConversionError> =>
  Effect.gen(function* () {
    ...
    if (rateFromBase === undefined) {
      return yield* Effect.fail(
        new CurrencyConversionError(
          fromCurrency,
          toCurrency,
          `No exchange rate available for ${fromCurrency}`,
        ),
      );
    }
...
const convertWithFallback = (
  ...
  convert(amount, fromCurrency, toCurrency).pipe(
    Effect.catchAll((error) => {
      console.warn(
        `Currency conversion failed, using 1:1 fallback: ${error.message}`,
      );
    ...
    effect: Effect.gen(function* () {
      return {
        ...
        convertWithFallback: (
          ...
          fromCurrency: string,
          toCurrency: string,
        ) => convertWithFallback(amount, fromCurrency, toCurrency),
         ...
         */
        getExponent: (currency: string) =>
          Effect.succeed(getExponent(currency)),
