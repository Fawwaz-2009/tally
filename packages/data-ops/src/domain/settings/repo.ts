    ...
    return {
      ...
      get: Effect.fn("settingsRepo.get")(function* () {
        const result = yield* withDbTryPromise(
          db.select().from(settingsTable).where(eq(settingsTable.id, SETTINGS_ID)).get()
        );
        ...
        return yield* withDbTryPromise(
          db.insert(settingsTable).values({ id: SETTINGS_ID }).returning().get()
        );
      }),

      // Update settings
      update: Effect.fn("settingsRepo.update")(function* (
        values: Partial<{ baseCurrency: string }>
      ) {
