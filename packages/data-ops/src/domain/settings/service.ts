...
export class SettingsService extends Effect.Service<SettingsService>()('SettingsService', {
  effect: Effect.gen(function* () {
    ...
    return {
      ...
      // Get base currency
      getBaseCurrency: Effect.fn('settingsService.getBaseCurrency')(function* () {
        const settings = yield* settingsRepo.get()
        return settings.baseCurrency
      }),

      // Set base currency
      setBaseCurrency: (currency: string) => settingsRepo.update({ baseCurrency: currency }),

      // Check if initial setup is complete
      isSetupComplete: Effect.fn('settingsService.isSetupComplete')(function* () {
      ...
      completeSetup: Effect.fn('settingsService.completeSetup')(function* (params: { userName: string; currency: string }) {
